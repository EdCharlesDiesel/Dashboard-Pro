...
def fetch_data(symbol: str, interval: str, period: str) -> pd.DataFrame:
    ...
    try:
        logger.info(f"Fetching {symbol} - {interval} - {period}")
...
class EntrySignalGenerator:
    ...
    def get_entry_signal(self, df_15m: pd.DataFrame, bias: str) -> Dict:
        ...
        if bias == 'Long':
            ...
            if self.config.london_start <= current_hour <= self.config.london_end or \
               self.config.ny_start <= current_hour <= self.config.ny_end:
                confidence += 1
        ...
        if swing_level is not None:
            ...
            if bias == 'Long':
                ...
                if struct_stop < atr_stop:
                    stop = struct_stop
                    method_parts.append(f"Swing Low ({swing_level:.5f})")
                else:
                    stop = atr_stop
                    method_parts.append(f"ATR ×{atr_mult:.1f}")
            else:
                struct_stop = swing_level + buffer
                if struct_stop > atr_stop:
                    stop = struct_stop
                    method_parts.append(f"Swing High ({swing_level:.5f})")
                else:
                    stop = atr_stop
                    method_parts.append(f"ATR ×{atr_mult:.1f}")
        else:
...
# ============================================================================
# TRADING IDEAS ENGINE - FIXED VERSION
# ============================================================================
...
def analyze_multi_timeframe_simple(df_daily, df_4h, df_1h, df_15m, pair_name):
    ...
    h4_trend = 'Long' if four_hour['Close'] > four_hour.get('EMA_20', four_hour['Close']) else 'Short'
    h1_trend = 'Long' if hourly['Close'] > hourly.get('EMA_20', hourly['Close']) else 'Short'
    
    # RSI signals
    daily_rsi = daily.get('RSI', 50)
    h4_rsi = four_hour.get('RSI', 50)
    h1_rsi = hourly.get('RSI', 50)
...
def main():
    ...
    with tab3:
        ...
        if available_pairs:
            ...
            if not df_15m.empty and not df_daily.empty:
                ...
                # Show 15m chart
                df_15m_ind = analyzer.add_indicators(df_15m)
                fig_15 = make_subplots(rows=2, cols=1, shared_xaxes=True, 
                                       row_heights=[0.7, 0.3])
                
                fig_15.add_trace(go.Candlestick(
                    x=df_15m_ind.index, open=df_15m_ind['Open'], high=df_15m_ind['High'],
                    low=df_15m_ind['Low'], close=df_15m_ind['Close'], name="15M"
                ), row=1, col=1)
                
                if 'Stoch_K' in df_15m_ind.columns:
                    fig_15.add_trace(go.Scatter(x=df_15m_ind.index, y=df_15m_ind['Stoch_K'], 
                                                name="Stoch K"), row=2, col=1)
                    fig_15.add_trace(go.Scatter(x=df_15m_ind.index, y=df_15m_ind['Stoch_D'], 
                                                name="Stoch D"), row=2, col=1)
                    fig_15.add_hline(y=80, line_dash="dash", line_color="red", row=2, col=1)
                    fig_15.add_hline(y=20, line_dash="dash", line_color="green", row=2, col=1)
    ...
    with tab4:
        ...
        st.caption("Multi-timeframe analysis with entry signals")
            ...
            if ideas:
                ...
                for idea in ideas:
                    with st.container():
                        ...
                        with cols[3]:
                            st.metric("R/R", f"1:{idea['risk_reward']:.2f}")
                        
                        st.caption(f"🛡️ Stop: {idea['stop_loss_method']} ({idea['stop_loss_pips']} pips)")
                        st.divider()
            else:
                st.warning("No trading ideas generated. Try again later or check data availability.")
