...
def generate_trading_ideas(df_day, bias, indicators):
    ...
    ema9 = last["EMA_9"] if last is not None and "EMA_9" in last else None # We can customize analyzer for these or use defaults
...
def build_chart(df_tf, bias, levels, tf_label, show_ema, show_bb, show_sessions, selected_date):
    ...
    if show_ema:
        for col, color in [("EMA_20", "#ff9800"), ("EMA_50", "#ab47bc")]:
            if col in df_tf.columns:
                fig.add_trace(go.Scatter(x=df_tf["datetime"], y=df_tf[col], line=dict(color=color, width=1.2), name=col), row=1, col=1)
    ...
    if "RSI" in df_tf.columns:
        fig.add_trace(go.Scatter(x=df_tf["datetime"], y=df_tf["RSI"], line=dict(color="#7986cb", width=1.5), name="RSI"), row=2, col=1)

    # MACD
    if "MACD" in df_tf.columns:
        fig.add_trace(go.Bar(x=df_tf["datetime"], y=df_tf["MACD_Histogram"], name="MACD Hist"), row=3, col=1)

    fig.update_layout(height=720, plot_bgcolor="#0e1117", paper_bgcolor="#0e1117", font=dict(color="#c0c0c0", size=11), xaxis_rangeslider_visible=False)
    return fig
