...
- **Pages auto-save their signals (system-wide).** Signal/scanner pages persist their reads to `trade_setups` through one shared service, **`src/services/signal_store.py`** → `persist_signals(source, signals)`. It owns every cross-cutting concern so a page is a ~3-line call: per-source dedupe (a `NotifyCache` JSON ledger keyed pair+bias+rounded-entry, surviving reruns/restarts), DB-target resolution via `market_cache._resolve_cfg()`, `source=<page>` tagging, pooled write, `clear_read_caches()` invalidation, and **graceful no-op** when the DB is down (without consuming the dedupe ledger, so a later configured run still saves). Signals are loose dicts (`pair`+`bias` required; `entry`/`stop_loss`/`take_profit_*`/`strength_score`/`conviction`/`thesis` optional) mapped by the pure, unit-tested `signals.signal_to_setup_row()` (ticker/pip_size from the registry; price targets → pips; sizing fields NULL; full context in `checks_detail` JSON). **Wired pages** (`source` tag): `market_overview`, `setup_ranker` (Grade-A), `weekly_swing`, `trend_signals`, `daily_trend`, `daily_macd`, `amd_scanner`, `confluence_checker`/`confluence_zone_4h` (non-directional → `bias='Neutral'`), `twenty_day_breakout` (open setups only), `vwap_ema_gold` (latest-bar only), `fib_entry`, `market_structure`, `weekly_ema`, `predictive` (price-vs-SMA), `forecast_dashboard` (only when the Monte-Carlo model shows an EDGE), `dxy_gold` (XAU/USD BUY/SELL read), `smart_money` (accumulation/distribution; free-text ticker), `risk_reversal` (contrarian read; **never on synthetic demo data**), `seasonality` (current-month bias). **Not wired** (no trade signal): calculators (`rr-calculator`, `account-risk`), views (`trade-journal`, `reports`, `system-logs`), `correlations`, `atr-volatility`, `stop-structure`, `news-filter`, `event_impact`, backtest/simulation pages (`double_zeros`, `backtest-workflow`), and the weak/ambiguous reads left off by choice (`weekly-rsi`, `macro-bias` [scores currencies not pairs], `regime`, `forex_fundamentals`, `trading_lab`). Each `cached_*` write is followed by cache invalidation — saving a signal busts the Redis journal cache so the journal/stats reflect it immediately. Note `signal_to_setup_row` caps `verdict` at the `trade_setups.verdict` VARCHAR(20) width.
- **DB caching/pooling lives in `src/db/cache.py`** (keeps the repository Streamlit-free). `pooled_repository(cfg)` borrows from a `@st.cache_resource` connection pool (one per DB target); the `cached_*` read fns are **Redis cache-aside** (`ttl=60`): check Redis → hit returns, miss queries Postgres → store in Redis → return. **Every write must call `clear_read_caches()`** (deletes the `dbcache:*` keys) so the journal/stats/open-trades/P&L re-query — you never cache a write, you invalidate after one. **Redis degrades gracefully**: if it's unreachable, reads fall straight through to Postgres and invalidation no-ops, so the app keeps working off the DB alone — never make Redis a hard dependency. The Redis target resolves from `[redis]` in secrets, else `REDIS_HOST`/`REDIS_PORT` env vars, else localhost:6379 (same convention as `src/data_backbone`). Schema init (`init_schema`) stays on a plain (lazy) `TradeRepository` so bad credentials surface as a message instead of crashing on eager pool creation.
- Secrets live in `.streamlit/secrets.toml` (gitignored): `[api]` FRED key, `[database]`, `[gmail]` (AMD scanner email alerts), optional `[redis]` (`host`/`port`). `.streamlit/config.toml` is theme-only and is tracked — never put keys in it.
